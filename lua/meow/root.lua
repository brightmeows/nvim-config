...
---@overload fun(): string
-- 行为源：LazyVim util/root.lua，改挂 meow 命名空间。
local LazyVim = require("meow")
...
function M.info()
  ...
  lines[#lines + 1] = "```"
  require("meow").info(lines, { title = "LazyVim Roots" })
...
function M.get(opts)
  ...
  return require("meow").is_win() and ret:gsub("/", "\\") or ret
end
